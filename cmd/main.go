...
func init() {
	...
	utilruntime.Must(koronerv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(restConfig, ctrl.Options{
		...
		LeaderElectionID:       "6f38856b.pez.sh",
	...
	// A typed clientset is required for evidence that the cached client cannot
	// serve: the pods/log subresource and field-selected event lists.
	clientset, err := kubernetes.NewForConfig(restConfig)
	if err != nil {
		setupLog.Error(err, "Failed to build kubernetes clientset")
		os.Exit(1)
	}
	collector := forensics.NewCollector(mgr.GetClient(), clientset)
